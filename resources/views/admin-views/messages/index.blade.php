            ...
            <div class="col-sm mb-2 mb-sm-0">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-no-gutter">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">{{\App\CentralLogics\translate('customer Messages')}}</li>
                    </ol>
                </nav>
    ...
    <div class="row">
        <div class="col-lg-4 col-4">
            <div class="input-group-overlay input-group-sm mb-1">
                <input style="background: aliceblue; border-radius: 15px" placeholder="Search user" class="cz-filter-search form-control form-control-sm appended-form-control" type="text" id="search-conversation-user" autocomplete="off">
            </div>
            <!-- Card -->
            <div class="card mb-3 mb-lg-5">
                <!-- Body -->
                <div class="card-body p-md-4 p-2" style="overflow-y: scroll;height: 600px" id="conversation_sidebar">
                    ...
                    <div class="sidebar_primary_div d-flex border-bottom pb-2 pt-2 pl-md-1 pl-0 justify-content-between align-items-center customer-list {{$unchecked!=0?'conv-active':''}}" onclick="viewConvs('{{route('admin.message.view',[$conv->user_id])}}','customer-{{$conv->user_id}}')" style="cursor: pointer; border-radius: 10px;margin-top: 2px;" id="customer-{{$conv->user_id}}">
                        <div class="avatar avatar-lg avatar-circle">
                            <img class="avatar-img" style="width: 54px;height: 54px" src="{{asset('storage/app/public/profile/'.$user['image'])}}" onerror="this.src='{{asset('public/assets/admin')}}/img/160x160/img1.jpg'" alt="Image Description">
                        </div>
...
<script>
    ...
    function viewConvs(url, id_to_active) {
        ...
        if (counter_element !== "undefined") {
            console.log('Inside bage remove: ' + '#counter-' + current_selected_user.slice(9));
        ...
        if (customer_element !== "undefined") {
            console.log('Inside conv remove: ' + '#' + current_selected_user);
            console.log(customer_element);
...
<script>
    ...
    if ('serviceWorker' in navigator) {
        var swRegistration = navigator.serviceWorker.register('{{ asset('
